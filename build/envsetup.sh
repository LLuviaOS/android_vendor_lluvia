function __print_LLUVIA_functions_help() {
cat <<EOF
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/LLuviaOS-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        if (adb shell getprop org.lluvia.device | grep -q "$LLUVIA_BUILD"); then
...
function githubremote()
{
    ...
    local REMOTE=$(git config --get remote.lluvia.projectname)

    local PROJECT=$(echo $REMOTE | sed -e "s#platform/#android/#g; s#/#_#g")

    git remote add github https://github.com/LLuviaOS/$PROJECT
...
function installboot()
{
    ...
    if (adb shell getprop org.lluvia.device | grep -q "$LLUVIA_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop org.lluvia.device | grep -q "$LLUVIA_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
if [ -d $(gettop)/prebuilts/snapdragon-llvm/toolchains ]; then
    case `uname -s` in
        ...
        *)
            ...
            export SDCLANG_LTO_DEFS=$(gettop)/vendor/lluvia/build/core/sdllvm-lto-defs.mk
            ;;
    esac
fi
